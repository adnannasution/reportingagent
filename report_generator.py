...
def gather_daily_data():
    data = {}
    data["bad_actor"] = q("""
        SELECT UPPER(TRIM(ru)) AS refinery_unit, tag_number, status, problem, action_plan, progress, target_date, periode
    ...
    """)
    data["icu"] = q("""
        SELECT UPPER(TRIM(ru)) AS refinery_unit, tag_no, icu_status, issue, mitigation, progress, target_closed, report_date
        FROM icu_monitoring
    ...
    """)
    data["zero_clamp"] = q("""
        SELECT UPPER(TRIM(ru)) AS refinery_unit, area, unit, tag_no_ln, type_damage, tanggal_dipasang, status
        FROM zero_clamp
        WHERE tanggal_dilepas IS NULL OR TRIM(COALESCE(tanggal_dilepas,'')) = ''
        ORDER BY tanggal_dipasang ASC NULLS LAST LIMIT 15
    """)
    data["paf"] = q("""
        SELECT UPPER(TRIM(ru)) AS refinery_unit, type, target_realisasi, value, color, month_update
        FROM paf
    ...
    data["power_utility"] = q("""
        SELECT UPPER(TRIM(refinery_unit)) AS refinery_unit, type_equipment, equipment, status_operation, remark, date_update
        FROM power_stream
    ...
    """)
    data["critical_utl"] = q("""
        SELECT UPPER(TRIM(refinery_unit)) AS refinery_unit, type_equipment, highlight_issue, corrective_action, target_corrective
    ...
    data["monitoring_operasi"] = q("""
        SELECT UPPER(TRIM(refinery_unit)) AS refinery_unit, unit_proses, actual, target_sts, limitasi_alert_process, month_update
...
def gather_weekly_data():
    ...
    data["bad_actor_prev"] = q("""
        SELECT UPPER(TRIM(ru)) AS refinery_unit, tag_number, status, progress FROM bad_actor_monitoring
        WHERE periode = (SELECT MAX(periode) FROM bad_actor_monitoring
                         WHERE periode < (SELECT MAX(periode) FROM bad_actor_monitoring))
        ORDER BY tag_number LIMIT 30
    """)
    data["icu_current"] = q("""
        SELECT UPPER(TRIM(ru)) AS refinery_unit, tag_no, icu_status, issue, progress, target_closed, report_date
        FROM icu_monitoring
        WHERE report_date = (SELECT MAX(report_date) FROM icu_monitoring)
          AND LOWER(COALESCE(icu_status,'')) NOT IN ('closed','resolved','selesai')
    ...
    """)
    data["paf_current"] = q("""
        SELECT UPPER(TRIM(ru)) AS refinery_unit, type, value, color, month_update FROM paf
        WHERE month_update = (SELECT MAX(month_update) FROM paf)
          AND LOWER(COALESCE(color,'')) IN ('red','yellow','orange','merah','kuning')
    ...
    data["readiness_jetty"] = q("""
        SELECT UPPER(TRIM(refinery_unit)) AS refinery_unit, tag_no, status_operation, status_tuks, status_ijin_ops, month_update
    ...
    data["readiness_tank"] = q("""
        SELECT UPPER(TRIM(refinery_unit)) AS refinery_unit, tag_number, status_operational, status_coi, status_atg, month_update
        FROM readiness_tank
    ...
    data["readiness_spm"] = q("""
        SELECT UPPER(TRIM(refinery_unit)) AS refinery_unit, tag_no, status_operation, status_laik_operasi, month_update
        FROM readiness_spm
    ...
    """)
    data["atg"] = q("""
        SELECT UPPER(TRIM(refinery_unit)) AS refinery_unit, tag_no_tangki, tag_no_atg, status_atg, status_interkoneksi_atg, month_update
        FROM atg_monitoring
...
def gather_monthly_data():
    ...
    """)
    data["icu_current"] = q("""
        SELECT UPPER(TRIM(ru)) AS refinery_unit, tag_no, icu_status, issue, progress, report_date FROM icu_monitoring
        WHERE report_date = (SELECT MAX(report_date) FROM icu_monitoring)
          AND LOWER(COALESCE(icu_status,'')) NOT IN ('closed','resolved','selesai')
        LIMIT 25
    """)
    ...
    """)
    data["readiness_jetty"] = q("""
        SELECT UPPER(TRIM(refinery_unit)) AS refinery_unit, tag_no, status_operation, status_tuks, month_update
        FROM readiness_jetty
        WHERE month_update = (SELECT MAX(month_update) FROM readiness_jetty)
          AND LOWER(COALESCE(status_operation,'')) NOT IN ('normal','siap','ok','ready')
        ORDER BY refinery_unit, tag_no LIMIT 15
    """)
    data["readiness_tank"] = q("""
        SELECT UPPER(TRIM(refinery_unit)) AS refinery_unit, tag_number, status_operational, status_coi, month_update
        FROM readiness_tank
        WHERE month_update = (SELECT MAX(month_update) FROM readiness_tank)
          AND LOWER(COALESCE(status_operational,'')) NOT IN ('normal','ok','siap')
        ORDER BY refinery_unit, tag_number LIMIT 15
